...
async def show_profile(call: CallbackQuery):
    ...
    text = (
        ...
        f"\n"
        f"<blockquote>"
        f"<tg-emoji emoji-id=\"5447425343570862600\">🥸</tg-emoji>Имя: {u[1]}\n"
        f"<tg-emoji emoji-id=\"5449907233372529609\">🌐</tg-emoji>Username: @{u[0] or '—'}\n"
        f"<tg-emoji emoji-id=\"5449740962303598934\">🔗</tg-emoji>ID: <code>{call.from_user.id}</code>\n"
        f"<tg-emoji emoji-id=\"5370867066188355513\">🪙</tg-emoji>Баланс: {u[2]} {cem} {cur}\n"
        f"<tg-emoji emoji-id=\"5449494323806626648\">🛍</tg-emoji>Рефералов: {u[3]}"
        f"</blockquote>\n"
        f"\n"
        f"<tg-emoji emoji-id=\"5371079538220487384\">💎</tg-emoji>Реферальная ссылка: {ref_link}"
    )
...
async def show_tasks(call: CallbackQuery, bot: Bot):
    ...
    if (await get_setting("tasks_op_enabled", "0")) == "1":
        ...
        if not_subbed:
            ...
            rows.append([InlineKeyboardButton(text="Я подписался", callback_data="op_check:tasks")])
            rows.append([InlineKeyboardButton(text="В меню", callback_data="nav:menu")])
            await send_section(
                call,
                "Подпишитесь на каналы для доступа к заданиям:",
                None,
    ...
    available_rows.append([InlineKeyboardButton(text="В меню", callback_data="nav:menu")])
...
async def cb_casino(call: CallbackQuery):
    ...
    text = (
        f"{emoji} <b>Казино</b>\n\n"
        f"🎲 Кубик: ставка {bet_d}, выигрыш {win_d}\n"
        f"🏀 Баскетбол: ставка {bet_b}, выигрыш {win_b}"
    )
...
async def cb_theft(call: CallbackQuery):
    ...
    text = (
        ...
        f"\n"
        f"<tg-emoji emoji-id=\"5449644694906626100\">🍀</tg-emoji>Шанс успеха: {chance}%\n"
        f"<tg-emoji emoji-id=\"5449644694906626100\">🍀</tg-emoji>При победе: +{win}% от баланса жертвы\n"
        f"<tg-emoji emoji-id=\"5370986844236305154\">💀</tg-emoji>При провале: -{lose}% от твоего баланса жертве"
    )
    ...
    text = (
        ...
        f"При победе: +{win}% от баланса жертвы\n"
        f"При провале: -{lose}% от твоего баланса жертве"
    )
    await send_section(call, text, "theft_photo", reply_markup=await theft_kb())
...
async def cb_rob(call: CallbackQuery):
    ...
    if me[0] <= 0:
        await call.answer("У вас 0 на балансе — нельзя грабить", show_alert=True); return
...
async def cb_lb(call: CallbackQuery):
    ...
    await send_section(call, f"{emoji} Топы:", "leaderboard_photo", reply_markup=await leaderboard_kb())
    return
    await call.answer()
    await send_section(call, "🏆 Топы:", "leaderboard_photo", reply_markup=await leaderboard_kb())
...
async def cb_lb_refs(call: CallbackQuery):
    await call.answer()
    emoji = await get_setting("section_emoji_leaderboard", "🏆")
    rows = await fetchall("SELECT full_name, referrals FROM users ORDER BY referrals DESC LIMIT 10")
    text = f"{emoji} <b>Топ по рефералам:</b>\n\n"
...
async def cb_lb_tokens(call: CallbackQuery):
    await call.answer()
    rows = await fetchall("SELECT full_name, balance FROM users ORDER BY balance DESC LIMIT 10")
    cur = await get_setting("currency_name")
    emoji = await get_setting("section_emoji_leaderboard", "🏆")
    text = f"{emoji} <b>Топ по {cur}:</b>\n\n"
